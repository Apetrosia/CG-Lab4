    ...
    public partial class Form1 : Form
    {
        ...
        private void helpingFunction(Point mousePoint, MouseEventArgs e)
        {
            if (isDeterminingPosition)
            {
                ...
                // Если точка уже есть, удаляем её
                if (pointExists)
                {
                    ClearPoint(); // Удаляем точку с экрана
                    pointExists = false;
                    clickedPoint = null; // Очищаем текущие координаты
                }
                else
                {
                    // Сохраняем координаты новой точки
                    clickedPoint = new Point(e.Location.X, e.Location.Y);
                    DrawPoint(clickedPoint.Value); // Рисуем точку
                    pointExists = true; // Устанавливаем флаг
            ...
            if (isDrawingEdgeForIntersection)
            {
                ...
                if (clickedPoints.Count == 2)
                {
                    ...
                    // Ребро, которое пользователь нарисовал
                    Point newEdgeStart = clickedPoints[0];
                    Point newEdgeEnd = clickedPoints[1];

                    // Проверяем пересечение с другими ребрами
